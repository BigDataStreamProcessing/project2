...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String  topic     = cfg.getProperty("kafka.topic.powietrze",  "powietrze-odczyty");
        long    interval  = Long.parseLong(cfg.getProperty("generator.interval.ms","10000"));
        double  anomaly   = Double.parseDouble(cfg.getProperty("generator.anomaly.probability","0.08"));
        long    disorder  = Long.parseLong(cfg.getProperty("generator.disorder.max.ms",        "0"));

        PowietrzeGenerator gen = new PowietrzeGenerator(anomaly, disorder);
        ...
        log.info("Start generatora jakosci powietrza | topic={}", topic);
        if (preview) {
            while (!Thread.currentThread().isInterrupted()) {
                AirQualityEvent e = gen.generate();

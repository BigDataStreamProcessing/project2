...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String  topic     = cfg.getProperty("kafka.topic.gielda",     "gielda-ticki");
        long    interval  = Long.parseLong(cfg.getProperty("generator.interval.ms","1000"));
        ...
        GieldaGenerator gen = new GieldaGenerator(anomaly, disorder);
        ...
        log.info("Start generatora gieldy | topic={}", topic);
        if (preview) {
            while (!Thread.currentThread().isInterrupted()) {
                CommodityTickEvent e = gen.generate();
            ...
            try (KafkaProducer<String, String> producer = new KafkaProducer<>(kp)) {
                while (!Thread.currentThread().isInterrupted()) {
                    CommodityTickEvent e = gen.generate();
                    producer.send(new ProducerRecord<>(topic, e.getContractId(), gen.toJson(e)));
                    Thread.sleep(interval);
                }
            }
        }

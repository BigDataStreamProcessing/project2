...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String  topic     = cfg.getProperty("kafka.topic.wodociagi",  "wodociagi-odczyty");
        long    interval  = Long.parseLong(cfg.getProperty("generator.interval.ms","8000"));
        ...
        WodociagiGenerator gen = new WodociagiGenerator(anomaly, disorder);
        ...
        log.info("Start generatora wodociagów | topic={}", topic);
        if (preview) {
            while (!Thread.currentThread().isInterrupted()) {
                WaterNetworkEvent e = gen.generate();
            ...
            try (KafkaProducer<String, String> producer = new KafkaProducer<>(kp)) {
                while (!Thread.currentThread().isInterrupted()) {
                    WaterNetworkEvent e = gen.generate();
                    producer.send(new ProducerRecord<>(topic, e.getNodeId(), gen.toJson(e)));
                    Thread.sleep(interval);
                }
            }
        }

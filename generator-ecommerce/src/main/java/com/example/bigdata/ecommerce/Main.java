...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String  topic     = cfg.getProperty("kafka.topic.ecommerce",  "ecommerce-transakcje");
        int     users     = Integer.parseInt(cfg.getProperty("generator.users.count",      "500"));
        long    interval  = Long.parseLong(cfg.getProperty("generator.interval.ms",         "2000"));
        double  anomaly   = Double.parseDouble(cfg.getProperty("generator.anomaly.probability","0.03"));
        long    disorder  = Long.parseLong(cfg.getProperty("generator.disorder.max.ms",        "0"));

        EcommerceGenerator gen = new EcommerceGenerator(users, anomaly, disorder);
        ...
        log.info("Start generatora e-commerce | topic={} | users={}", topic, users);
        if (preview) {
            while (!Thread.currentThread().isInterrupted()) {
                OrderEvent e = gen.generate();
            ...
            try (KafkaProducer<String, String> producer = new KafkaProducer<>(kp)) {
                while (!Thread.currentThread().isInterrupted()) {
                    OrderEvent e = gen.generate();
                    producer.send(new ProducerRecord<>(topic, e.getUserId(), gen.toJson(e)));
                    Thread.sleep(interval);
                }
            }
        }

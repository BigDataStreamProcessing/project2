...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String  topic     = cfg.getProperty("kafka.topic.maszyny",    "maszyny-telemetria");
        long    interval  = Long.parseLong(cfg.getProperty("generator.interval.ms","4000"));
        ...
        MaszynyGenerator gen = new MaszynyGenerator(anomaly, disorder);
        ...
        log.info("Start generatora maszyn | topic={}", topic);
        if (preview) {
            while (!Thread.currentThread().isInterrupted()) {
                MachineEvent e = gen.generate();
            ...
            try (KafkaProducer<String, String> producer = new KafkaProducer<>(kp)) {
                while (!Thread.currentThread().isInterrupted()) {
                    MachineEvent e = gen.generate();
                    producer.send(new ProducerRecord<>(topic, e.getMachineId(), gen.toJson(e)));
                    Thread.sleep(interval);
                }
            }
        }
